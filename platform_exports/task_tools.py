...
class task:
    ...
    def __str__(self) -> str:
        """
        The string representation of the task is equal to the name.
        """
        return self.name_ + "," + str(self.id_)
    ...
    def get_relation(self, other_task: 'task') -> task_relation:
        ...
        """
        if self.task_relations_ is None:
            return None
        elif other_task not in self.task_relations_.keys():
            return None
        else:
...
class file_task_builder(task_builder):
    ...
    def from_file(self, file: io.TextIOWrapper) -> list[task]:
        ...
        for line in csv_reader:
            ...
            relation_tasks_ids[current_task] = self._get_list(line[file_task_builder.relation_tasks_str], file_task_builder.list_del_str)
...
def get_common_period(tasks : list[task]) -> int:
    """
    Get the common scheduling period for the tasks.
    Requires a common timing resolution!
    """
    
    # Check the timing resolution and raise an error if it is not the same
    last_timing_resolution = tasks[0].get_resolution()
    for i in range(1,len(tasks)):
        current_resolution = tasks[i].get_resolution()
        if last_timing_resolution == current_resolution:
            last_timing_resolution = current_resolution
        else:
            raise RuntimeError("Timing resolution is not same! Previous: " + str(last_timing_resolution) + ". This: " + str(current_resolution))
        
    # Calculate least common multiple after:
    ...
    for t in tasks:
        if t.get_periodic_deadline() is not None and t.get_periodic_deadline() != -1:
            dl = t.get_periodic_deadline()
        else:
            dl = max(t.exec_times_.values())
